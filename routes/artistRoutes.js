...
// GET /api/artists/top - Fetch top 3 artists by sales volume
router.get("/top", async (req, res) => {
  try {
    const db = req.app.get("db");
    const artists = await db.collection("users")
      .find({ role: "artist" })
      .sort({ totalSold: -1 })
      .limit(3)
      .project({ password: 0 })
...
// GET /api/artists - Browse artists with optional query filters
...
// GET /api/artists/:id - Fetch profile for a single artist
...
// GET /api/artists/:id/stats - Retrieve artist portfolio aggregated performance metrics
